...
class system_sync:
    ...
    def __init__(self, config_file=None):
        ...
        if config_file is None:
            ...
            raise TypeError(f'設定ファイル({config_file})が文字列ではありません。')
        elif len(config_file) < 1:
            raise TypeError(f'設定ファイル({config_file})が空文字列です。')
        else:
            self.config_file = pathlib.Path(config_file).resolve()
        ##########################################################################
        if not pathlib.Path(self.config_file).exists():
            raise FileNotFoundError(f'設定ファイル({config_file})が存在しません。')
        elif not pathlib.Path(self.config_file).is_file():
            raise FileNotFoundError(f'設定ファイル({config_file})がファイルではありません。')
        elif pathlib.Path(self.config_file).stat().st_size < 1:
            raise KeyError(f'設定ファイル({config_file})の中身が空です。')
        ##########################################################################
        self.config = {}
    ...
    def ping(self, server, retryCount=1):
        ...
        try:
            subprocess.run(['ping', '-c', str(retryCount), server], check=True, stdout=subprocess.DEVNULL,
    ...
    def getAllMachineInfo(self):
        ...
        for machine in self.allMachineInfo:
            if machine['type'] == 'master':
                self.sqlMasterConfig['host'] = machine['name']
                break
        ##########################################################################
    ...
    def doCommand(self, machine_id, command, data={}, sub_data={}):
        ...
        if type(machine_id) is not int:
            raise TypeError(f'machine_id ({machine_id}) が有効ではではありません。')
        elif machine_id < 1:
            raise KeyError(f'machine_id ({machine_id}) が有効ではではありません。')
        ...
        if type(command) is not dict:
            raise TypeError(f'command ({command}) が有効ではではありません。')
        elif 'command' not in command:
            raise KeyError(f'command ({command}) が有効ではではありません。')
        ...
        if type(data) is not dict:
            raise TypeError(f'data ({data}) が有効ではではありません。')
            ##########################################################################
        ...
        for match in pattern.finditer(command['command']):
            ...
            if key in data:
                ...
                raise KeyError(f'command ({command}) の key ({key}) が有効ではではありません。')
                ##########################################################################
        if len(args) < 1:
        ...
        if self.machineInfo['id'] == machine_id:
            ...
            result = subprocess.run(commandList, shell=command['is_shell_subprocess']).returncode == 0 or command[
                'allow_error']
        else:
            ##########################################################################
            # ssh で実行
            ...
            if packageInitialize['unit_name'] is None:
                ...
                if len(packedPakageInitializeList) > 0:
                    if packedPakageInitializeList[-1]['unit_name'] != packageInitialize['unit_name']:
                        ...
                        print(f'WARNING : Duplicate unit ({packageInitialize["unit_name"]})', file=sys.stderr)
                        continue
                else:
                    packedPakageInitializeList.append(packageInitialize)
                    continue
                ...
                if packedPakageInitializeList[-1]['unit_name'] is None:
                    print(
                        f'WARNING : ({packageInitialize["name"]}) Parent unit ({packageInitialize["unit_name"]}) NOT FOUND',
                        file=sys.stderr)
                    continue
                ...
                if packedPakageInitializeList[-1]['unit_name'] == packageInitialize['unit_name']:
                    ...
                    ##########################################################################
                    print(
                        f'パッケージ依存エラー : WARNING : ({ackageInitialize["package"]}) Parent unit ({packageInitialize["unit_name"]}) NOT FOUND',
                        file=sys.stderr)
                    continue
            else:
                ##########################################################################
                ...
                ##########################################################################
                print(
                    f'WARNING : ({packageInitialize["package"]}) Parent unit ({packageInitialize["unit_name"]}) NOT FOUND',
                    file=sys.stderr)
                continue
        ##########################################################################
        # 修了
    ...
    def initializeMachine(self, machine_id=None):
        ...
        for packageInitialize in packageInitializeList:
            ...
            if packageInitialize['unit_name'] is not None:
                self.cursor.execute(system_sync.commandUnitPreInitializeSQL,
                                    {'unit_name': packageInitialize['unit_name'], 'machine_id': machine_id})
            ...
            ##########################################################################
            packageInitialize['initialized'] = self.installPackage(machine_id,
                                                                   packageInitialize['package']) and preUnitInitialize
            if 'sub_package' in packageInitialize and len(packageInitialize['sub_package']) > 0:
                for subPackage in packageInitialize['sub_package']:
                    packageInitialize['initialized'] = self.installPackage(machine_id, subPackage['package']) and \
                                                       packageInitialize['initialized']
            ##########################################################################
            ...
            ##########################################################################
            self.cursor.execute(system_sync.insertPackageInitializeSQL,
                                {'package_id': packageInitialize['package_id'], 'machine_id': machine_id,
                                 'initialized': packageInitialize['initialized']})
            if 'sub_package' in packageInitialize and len(packageInitialize['sub_package']) > 0:
                for subPackage in packageInitialize['sub_package']:
                    self.cursor.execute(system_sync.insertPackageInitializeSQL,
                                        {'package_id': subPackage['package_id'], 'machine_id': machine_id,
                                         'initialized': packageInitialize['initialized']})
        ##########################################################################
        return True
    ...
    def dumpDatabase(self, machine_id):
        ...
        for machine in self.allMachineInfo:
            if machine['type'] == 'master':
                ...
                break
        if host is None:
            raise KeyError(f'allMachineInfo ({host}) master のレコードが存在しません。')
        ##########################################################################
        ...
        with dumpAbsPath.open(mode='w') as file:
            ...
            with subprocess.Popen(
                    ['mariadb', '--user=' + self.databaseDump['user'], '--password=' + self.databaseDump['password'],
                     '--batch', '--skip-column-names', '-e', 'show databases'],
                    stdout=subprocess.PIPE) as databaseListProcess:
            ...
            ##########################################################################
            with subprocess.Popen(
                    ['mysqldump', '--user=' + self.databaseDump['user'], '--password=' + self.databaseDump['password'],
                     '--all-databases', '--add-drop-database', '--add-drop-table', '--master-data', '--gtid',
                     '--hex-blob', '--single-transaction', '--flush-privileges'],
                    stdout=subprocess.PIPE) as databaseDumpProcess:
    ...
    ##########################################################################
    def writeDatabaseServerConfig(self, machine_id=None, databaseServerConfigFile=None,
                                  writeDatabaseServerConfigPath=None, writeToFile=True):
        """データベースの設定ファイルを編集します。"""
        ##########################################################################
        ...
        ##########################################################################
        if not self.isMaster():
            raise KeyError(f'({self.machineInfo}) マスターサーバー以外から呼び出されました。')
        ##########################################################################
        if machine_id is None:
            machine_id = self.machineInfo['id']
        ##########################################################################
        ...
        if not databaseServerConfigAbsPath.is_file():
            raise FileNotFoundError(f'設定ファイル({databaseServerConfigAbsPath})が存在しません')
        ...
        if config.has_section('mysqld'):
            ...
            raise KeyError(f'設定ファイル({databaseServerConfigAbsPath})にサーバー設定がが存在しません')
        ##########################################################################
    ...
    def installPackage(self, machine_id, packageName):
        ...
        ##########################################################################
        if type(machine_id) is not int:
            raise TypeError(f'machine_id({machine_id})が整数ではありません。')
        elif machine_id < 1:
            raise KeyError(f'machine_id({machine_id})が整数ではありません。')
        elif type(packageName) is not str:
            raise TypeError(f'packageName({packageName})が文字列ではありません。')
        elif len(packageName) < 1:
            raise TypeError(f'packageName({packageName})が空です。')
        ##########################################################################
        # 全マシン初期化情報の取得
        ##########################################################################
        if not hasattr(self, 'allInitializeInfo'):
        ...
        if packageInfo is None:
            raise KeyError(f'({packageName}) パッケージが見つかりません。')
        ##########################################################################
        # パッケージインストールコマンドの取得
        ##########################################################################
        ...
        if packageInstallCommand is None:
            raise KeyError(f'({packageInstallCommand}) パッケージインストールコマンドが見つかりません。')
        ##########################################################################
        # パッケージインストールコマンドの実行
        ##########################################################################
    ...
    def doUnitCommand(self, machine_id, unit_name):
        ...
        if type(machine_id) is not int:
            ...
            raise TypeError(f'unit_name({unit_name})が文字列ではありません。')
        elif len(unit_name) < 1:
            raise TypeError(f'unit_name({unit_name})が空です。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        for commandUnitInitialize in commandUnitInitializeList:
            if not self.doCommand(machine_id, commandUnitInitialize,
                                  {'unit_name': unit_name, 'unit': unit_name, 'machine_id': machine_id}):
                return False
        ##########################################################################
        return True
    ...
    def doUnitSatrt(self, package):
        ...
        if type(package) is not dict:
            raise TypeError(f'package({package})が辞書型ではありません。')
        elif 'unit_name' not in package:
            raise TypeError(f'package({package})にユニット名がありません。')
        elif package['unit_name'] is None:
            return True
        elif 'need_enable' not in package:
            raise TypeError(f'package({package})に need_enable がありません。')
        elif 'need_start' not in package:
            raise TypeError(f'package({package})に need_start がありません。')
        elif 'machine_id' not in package:
            raise TypeError(f'package({package})に machine_id がありません。')
        elif 'package_id' not in package:
            raise TypeError(f'package({package})に package_id がありません。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        if package['need_enable']:
            ...
            if unitEnableCommand is None:
                raise KeyError(f'({str(self.machineInfo)}) unit enable コマンドが見つかりません。')
            ##########################################################################
            # ユニット自動起動コマンドの実行
            ##########################################################################
            result = self.doCommand(machine_id, unitEnableCommand,
                                    {'unit_name': package['unit_name'], 'unit': package['unit_name'],
                                     'machine_id': machine_id}) and result
        ##########################################################################
        ...
        if package['need_start']:
            ...
            if unitStartCommand is None:
                raise KeyError(f'({str(self.machineInfo)}) unit start コマンドが見つかりません。')
            ##########################################################################
            # ユニット開始コマンドの実行
            ##########################################################################
            result = self.doCommand(machine_id, unitStartCommand,
                                    {'unit_name': package['unit_name'], 'unit': package['unit_name'],
                                     'machine_id': machine_id}) and result
            ##########################################################################
            # ユニット開始後コマンドの取得
            ##########################################################################
            self.cursor.execute(system_sync.commandUnitAfterStartSQL,
                                {'machine_id': machine_id, 'unit_name': package['unit_name']})
        ...
        if self.isMaster():
            for dataFile in filesList:
                ...
                if not path.exists():
                    ...
                    if isText:
                        self.uploadTextFile(filePath=dataFile['path'], unit=dataFile['unit'],
                                            isDatabaseConfig=dataFile['is_database_config'])
                    else:
                        self.uploadBinaryFile(filePath=dataFile['path'], unit=dataFile['unit'])
    ...
    def getUnitText(self, machine_id, unit, is_database_setup=False):
        ...
        if unit is None:
            ...
            raise TypeError(f'unit({unit})が文字列ではありません。')
        elif len(unit) < 1:
            return []
        ##########################################################################
        if type(machine_id) is not int:
        ...
        if self.isMaster():
            for textFile in filesList:
                ...
                if not path.exists():
                    ...
                    # 設定ファイルが更新されているのでアップロード
                    self.uploadTextFile(filePath=textFile['path'], unit=unit,
                                        isDatabaseConfig=textFile['is_database_config'])
                    with path.open() as file:
    ...
    def writeToFile(self, filesList):
        ...
        if type(filesList) is not list:
            raise TypeError(f'filesList({filesList})がリストではありません。')
        if len(filesList) < 1:
            return True
        if type(filesList[0]) is not dict:
            raise TypeError(f'filesList({filesList})がリストの中身が不正です。')
        if 'machine_id' not in filesList[0]:
            raise KeyError(f'filesList({filesList})がリストが正しくありませんありません。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        for dataFile in filesList:
            ...
            if 'machine_id' not in dataFile:
                raise KeyError(f'dataFile({dataFile})が正しくありませんありません。')
            ...
            if dataFile['type'] == 'security':
                ...
                if securityInstallCommand is None:
                    raise KeyError(f'({targetMachine}) セキュリティインストールコマンドが見つかりません。')
            ##########################################################################
            if self.machineInfo['id'] == machine_id:
                path = pathlib.Path(dataFile['path'])
                if not path.exists():
                    ...
                    if 'is_database_config' in dataFile and dataFile['is_database_config'] and dataFile[
                        'mime_type'].lower().startswith('text'):
                        # MySQL/MariaDB Server setting config-file update
                        result = self.writeDatabaseServerConfig(machine_id=machine_id,
                                                                databaseServerConfigFile=dataFile['path'],
                                                                writeDatabaseServerConfigPath=dataFile[
                                                                    'path']) and result
                elif dataFile['last_modify'] > datetime.datetime.fromtimestamp(path.stat().st_mtime):
                    path.chmod(dataFile['file_mode'])
                    os.chown(path, pwd.getpwnam(dataFile['file_owner']).pw_uid,
                             grp.getgrnam(dataFile['file_group']).gr_gid)
                    with path.open(mode=writeMode) as file:
                        file.write(dataFile['data'])
                    if 'is_database_config' in dataFile and dataFile['is_database_config'] and dataFile[
                        'mime_type'].lower().startswith('text'):
                ...
                if securityInstallCommand is not None:
                    if not self.doCimmand(machine_id, securityInstallCommand, dataFile):
                        raise OSError(errno.EIO, f'({securityInstallCommandStr})セキュリティのインストールに失敗しました。')
                        return False
                ##########################################################################
            elif not self.ping(targetMachine['name']):
                raise OSError(errno.EHOSTUNREACH, f'ホスト({targetMachine["name"]})への接続ができません。')
                return False
            else:
                (fd, pathStr) = tempfile.mkstemp(text=True)
                pathlib.Path(pathStr).chmod(dataFile['file_mode'])
                os.chown(pathStr, pwd.getpwnam(dataFile['file_owner']).pw_uid,
                         grp.getgrnam(dataFile['file_group']).gr_gid)
                ...
                if 'is_database_config' in dataFile and dataFile['is_database_config'] and dataFile[
                    'mime_type'].lower().startswith('text'):
                    # MySQL/MariaDB Server setting config-file update
                    result = self.writeDatabaseServerConfig(machine_id=machine_id, databaseServerConfigFile=pathStr,
                                                            writeDatabaseServerConfigPath=dataFile['path'])
                else:
                    result = self.scp(targetMachine['id'], pathStr, str(dataFile['path']))
                pathlib.Path(pathStr).unlink()
                if not result:
                    raise OSError(errno.ECONNREFUSED, f'ホスト({targetMachine["name"]})への転送が失敗しました。{dataFile["path"]}')
                    return False
                ##########################################################################
                if securityInstallCommand is not None:
                    if not self.doCommand(machine_id, securityInstallCommand, dataFile):
                        raise OSError(errno.EREMOTEIO, f'({securityInstallCommandStr})セキュリティのインストールに失敗しました。over ssh')
                        return False
                ##########################################################################
            ##########################################################################
        return result
    ...
    def scp(self, machine_id, srcPath, destPath):
        ...
        if type(machine_id) is not int:
            ...
            raise TypeError(f'srcPath({srcPath})が文字列ではありません。')
        elif len(srcPath) < 1:
            raise TypeError(f'srcPath({srcPath})が空です。')
        elif type(destPath) is not str:
            raise TypeError(f'destPath({destPath})が文字列ではありません。')
        elif len(destPath) < 1:
            raise TypeError(f'destPath({destPath})が空です。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        for command in self.cursor.fetchall():
            pass
        if command is None:
            raise KeyError(f'({self.machineInfo}) scp コマンドが見つかりません。')
        ##########################################################################
        scpStr = command['command'].format(src=srcPath, user=self.sections['scp']['user'], host=targetMachine['name'],
                                           dest=destPath)
        scp = pexpect.spawn(scpStr, encoding='utf-8')
        while True:
            index = scp.expect([system_sync.passwordRegexpStr, system_sync.sshKeyRegExpStr, pexpect.EOF],
                               timeout=self.sections['expect']['timeout'])
            if index == 0:
    ...
    ##########################################################################
    def ssh(self, machine_id, commandStr, useLocalSubprocess=True):
        ...
        if type(machine_id) is not int:
            ...
            raise TypeError(f'commandStr({commandStr})が文字列ではありません。')
        elif len(commandStr) < 1:
            raise TypeError(f'commandStr({commandStr})が空です。')
        elif type(useLocalSubprocess) is not bool:
            raise TypeError(f'useLocalSubprocess({useLocalSubprocess})が Boolean ではありません。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        if targetMachine is None:
            raise KeyError(f'machine_id ({machine_id}) の値が正しくありません。')
        ##########################################################################
        if useLocalSubprocess is True and machine_id == self.machineInfo['id']:
        ...
        for command in self.cursor.fetchall():
            pass
        if command is None:
            raise KeyError(f'({elf.machineInfo}) ssh コマンドが見つかりません。')
        ##########################################################################
        sshStr = command['command'].format(user=self.sections['ssh']['user'], host=targetMachine['name'],
                                           command=commandStr)
        sshList = sshStr.split(maxsplit=command['maxsplit'])
        ssh = pexpect.spawn(sshList[0], sshList[1:], encoding='utf-8')
        while True:
            index = ssh.expect([system_sync.passwordRegexpStr, system_sync.sshKeyRegExpStr, pexpect.EOF],
                               timeout=self.sections['expect']['timeout'])
            if index == 0:
    ...
    def rsync(self, machine_id, pathStr):
        ...
        if type(machine_id) is not int:
            ...
            raise TypeError(f'pathStr({pathStr})が文字列ではありません。')
        elif len(pathStr) < 1:
            raise TypeError(f'pathStr({pathStr})が空です。')
        elif not pathlib.Path(pathStr).exists():
            raise KeyError(f'pathStr({pathStr})が存在していません。')
        ##########################################################################
        # 全マシン情報の取得
        ##########################################################################
        if not hasattr(self, 'allMachineInfo'):
        ...
        for command in self.cursor.fetchall():
            pass
        if command is None:
            raise KeyError(f'({self.machineInfo}) rsync コマンドが見つかりません。')
        ##########################################################################
        rsyncStr = command['command'].format(src=pathStr, user=self.sections['rsync']['user'],
                                             host=targetMachine['name'], dest=pathStr)
        print(rsyncStr)
        rsyncList = rsyncStr.split(maxsplit=command['maxsplit'])
        rsync = pexpect.spawn(rsyncList[0], rsyncList[1:], encoding='utf-8')
        while True:
            index = rsync.expect([system_sync.passwordRegexpStr, system_sync.sshKeyRegExpStr, pexpect.EOF],
                                 self.sections['expect']['timeout'])
            ...
            if index == 0:
                ...
                if result:
                    self.cursor.execute(system_sync.rsyncListSQL,
                                        {'os_id': self.machineInfo['os_id'], 'directory': pathStr})
                    rsync = None
                    for rsync in self.cursor.fetchall():
                        pass
                    if rsync is None:
                        raise KeyError(f'({pathStr}) ({elf.machineInfo["os_id"]}) rsync 情報が見つかりません。')
                    self.cursor.execute(system_sync.insertRsyncDateSQL,
                                        {
                                            'rsync_id': rsync['id'],
                                            'machine_id': machine_id,
