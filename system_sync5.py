...
class system_sync:
    ...
    def dumpDatabase( self, machine_id ):
        ...
        with dumpAbsPath.open( mode='w' ) as file:
            ...
            # DATABASE DUMP
            ##########################################################################
            with subprocess.Popen( [ 'mysqldump', '--user=' + self.databaseDump['user'], '--password=' + self.databaseDump['password'], '--all-databases', '--add-drop-database', '--add-drop-table', '--master-data', '--gtid', '--hex-blob', '--single-transaction' ], stdout=subprocess.PIPE ) as databaseDumpProcess:
