...
class system_sync:
	...
	##########################################################################
	configFile = '/usr/local/etc/system_sync/system_sync.conf'
	masterConfFile	= '/usr/local/etc/system_sync/system_mastaer_connectors.cnf'
	defaultConfFile	= '/usr/local/etc/system_sync/system_connectors.cnf'
	##########################################################################
	def __init__( self, config_file=None ):
		'''このクラスは、同じサーバー何にある mariadb に接続し、同期を行います。'''
		##########################################################################
		if config_file is None:
			self.config_file = system_sync.configFile
		...
		for key in self.default.keys():
			...
			if value.lower() in [ 'yes', 'true', '1' ]:
				if key.lower().endswith('_id') or key.lower().endswith('_no') or key.lower().endswith('_num') or key.lower().endswith('port'):
					value = int( value )
				else:
					value = True
		...
		for packageInitialize in packageInitializeList:
			...
			if packageInitialize['unit_name'] is None:
				...
				if packedPakageInitializeList[-1]['unit_name'] is None:
					print( 'WARNING : Parent unit ({unit}) NOT FOUND'.format( unit = packageInitialize['unit_name'] ), file = sys.stderr )
					continue
	...
	def dumpDatabase( self, machine_id ):
		...
		with dumpAbsPath.open( mode='w' ) as file:
			...
			# DATABASE DUMP
			##########################################################################
			with subprocess.Popen( [ 'mysqldump', '--user=' + self.databaseDump['user'], '--password=' + self.databaseDump['password'], '--all-databases', '--add-drop-database', '--add-drop-table', '--master-data', '--gtid', '--hex-blob', '--ignore-database=information_schema', '--ignore-database=performance_schema', '--single-transaction' ], stdout=subprocess.PIPE ) as databaseDumpProcess:
		...
		if self.isMaster():
			for binaryFile in filesList:
				...
				if not path.exists():
					...
					# 設定ファイルが更新されているのでアップロード
					self.uploadBinaryFile( filePath = binaryFile['path'], unit = unit, isDatabaseConfig = binaryFile['is_database_config'] )
